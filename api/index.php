...
function menuRow(array $r, array $bestsellerIds = []): array { $r['id']=(int)$r['id']; $r['isVeg']=(bool)$r['isVeg']; $r['isAvailable']=(bool)$r['isAvailable']; $r['categoryId']=(int)$r['categoryId']; $r['price']=(float)$r['price']; $r['image']=projectUrl((string)($r['image'] ?? '')); $r['category']=['id'=>(int)$r['categoryId'],'name'=>$r['categoryName']]; unset($r['categoryName']); $r['bestseller']=in_array($r['id'],$bestsellerIds,true); return $r; }
...
function createAdminOrder(array $b): never {
  auth('admin');
  $mobileNumber = mobile($b['mobileNumber'] ?? '');
  if (strlen($mobileNumber) < 10) throw new RuntimeException('A valid mobile number is required');
  $customerName = trim((string)($b['customerName'] ?? ''));
  if ($customerName === '') throw new RuntimeException('Customer name is required');
  $customer = customerFor($mobileNumber, $customerName);
  $pdo = db(); $pdo->beginTransaction();
  try {
    $items = orderItemsFromPayload($b['items'] ?? []);
    $subtotal = array_sum(array_column($items, 'subtotal'));
    $orderType = in_array($b['orderType'] ?? '', ['DINE_IN', 'TAKEAWAY', 'DELIVERY'], true) ? $b['orderType'] : 'DINE_IN';
    $paymentMethod = in_array($b['paymentMethod'] ?? '', ['CASH', 'UPI', 'CARD'], true) ? $b['paymentMethod'] : 'CASH';
    $delivery = deliveryChargeForOrder($subtotal, $orderType, (string)($b['locality'] ?? ''), (string)($b['sector'] ?? ''), (string)($b['customLocation'] ?? ''));
    ...
    $grand = money($foodBillAfterCashback + $delivery);
    $number = 'ORD-' . round(microtime(true) * 1000);
    $session = bin2hex(random_bytes(24));
    stmt('INSERT INTO `order`(orderNumber,customerId,customerName,mobileNumber,address,tableNumber,orderType,paymentMethod,totalAmount,gstAmount,discountAmount,cashbackRedeemed,cashbackEarned,grandTotal,status,confirmedAt,customerSessionToken,customerSessionExpiresAt) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,NOW(),?,DATE_ADD(NOW(),INTERVAL 30 MINUTE))',
      [$number, $customer['id'], $customerName, $mobileNumber, $b['address'] ?? null, $b['tableNumber'] ?? null, $orderType, $paymentMethod, $subtotal, $gst, $discount, $redeemed, $earned, $grand, 'PENDING', $session]);
